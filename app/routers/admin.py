...
from app.services.calificacion_service import calificar_hojas_pendientes
...
async def dashboard(
    ...
    rol = usuario.get('rol', 'COORDINADOR')
    proceso = obtener_proceso_actual()
    
    if rol == 'PROFESOR_CUIDADOR':
        return await dashboard_profesor(request, db, usuario, proceso)
    elif rol == 'COMISION':
        return await dashboard_comision(request, db, usuario, proceso)
    elif rol == 'DIRECTOR':
        return await dashboard_director(request, db, usuario, proceso)
    else:
        # COORDINADOR o cualquier otro
        return await dashboard_coordinador(request, db, usuario, proceso)


# ============================================================================
# DASHBOARD COORDINADOR
# ============================================================================
async def dashboard_coordinador(request: Request, db: Session, usuario: dict, proceso: str):
    """Dashboard para COORDINADOR - gestión completa"""
...
# DASHBOARD PROFESOR
# ============================================================================
async def dashboard_profesor(request: Request, db: Session, usuario: dict, proceso: str):
    """Dashboard para PROFESOR_CUIDADOR"""
    
    # Obtener aula asignada al profesor
    aula_result = db.execute(text("""
        SELECT 
            a.id,
            a.codigo,
            a.nombre,
            a.pabellon,
            a.capacidad,
            (SELECT COUNT(*) FROM postulantes p WHERE p.aula_id = a.id AND p.proceso_admision = :proceso) as asignados,
            (SELECT COUNT(*) FROM hojas_respuestas hr 
             JOIN postulantes p ON hr.postulante_id = p.id 
             WHERE p.aula_id = a.id AND hr.proceso_admision = :proceso 
             AND hr.estado IN ('PROCESADO', 'procesado')) as capturados
        FROM aulas a
        JOIN asignacion_profesor_aula apa ON a.id = apa.aula_id
        WHERE apa.profesor_id = :profesor_id AND apa.proceso_admision = :proceso
        LIMIT 1
    """), {"profesor_id": usuario.get('profesor_id'), "proceso": proceso}).fetchone()
    
    aula = {}
    if aula_result:
        asignados = aula_result.asignados or 0
        capturados = aula_result.capturados or 0
        aula = {
            "id": aula_result.id,
            "codigo": aula_result.codigo,
            "nombre": f"{aula_result.pabellon} - {aula_result.nombre}" if aula_result.pabellon else aula_result.nombre,
            "capacidad": aula_result.capacidad,
            "asignados": asignados,
            "capturados": capturados,
            "pendientes": max(0, asignados - capturados),
            "porcentaje": round((capturados / asignados * 100) if asignados > 0 else 0)
        }
    else:
        aula = {
            "nombre": "Sin aula asignada",
            "asignados": 0,
            "capturados": 0,
            "pendientes": 0,
            "porcentaje": 0
    ...
    revisiones_result = db.execute(text("""
        SELECT COUNT(*) as total
        FROM hojas_respuestas hr
        JOIN postulantes p ON hr.postulante_id = p.id
        WHERE p.aula_id = :aula_id 
        AND hr.proceso_admision = :proceso
        AND hr.requiere_revision = true
    """), {"aula_id": aula.get('id'), "proceso": proceso}).fetchone()
    
    revisiones_pendientes = revisiones_result.total if revisiones_result else 0
    
    return templates.TemplateResponse(
        "admin/dashboard_profesor.html",
        {
            "request": request,
            "usuario": usuario,
            "proceso_actual": proceso,
...
# DASHBOARD COMISION
# ============================================================================
async def dashboard_comision(request: Request, db: Session, usuario: dict, proceso: str):
    """Dashboard para COMISION - carga de gabarito"""
    ...
    total = result.total_postulantes or 0
    procesadas = result.hojas_procesadas or 0
    pendientes = max(0, total - procesadas)
    porcentaje = round((procesadas / total * 100) if total > 0 else 0)
    gabarito_cargado = (result.gabarito_count or 0) == 100
    
    # Puede cargar gabarito si hay al menos 1 hoja procesada y gabarito no existe
    puede_cargar = procesadas > 0 and not gabarito_cargado
    
    stats = {
        "total_postulantes": total,
        "hojas_procesadas": procesadas,
        "hojas_pendientes": pendientes,
        "porcentaje_avance": porcentaje,
        "gabarito_cargado": gabarito_cargado,
        "puede_cargar_gabarito": puede_cargar
    }
    
    return templates.TemplateResponse(
        "admin/dashboard_comision.html",
        {
            "request": request,
            "usuario": usuario,
            "proceso_actual": proceso,
...
# DASHBOARD DIRECTOR
# ============================================================================
async def dashboard_director(request: Request, db: Session, usuario: dict, proceso: str):
    """Dashboard para DIRECTOR - vista ejecutiva y publicación"""
    ...
    total = result.total_postulantes or 0
    procesadas = result.hojas_procesadas or 0
    calificadas = result.hojas_calificadas or 0
    
    stats = {
        "total_postulantes": total,
        "hojas_procesadas": procesadas,
        "hojas_completas": procesadas >= total and total > 0,
        "gabarito_cargado": (result.gabarito_count or 0) == 100,
        "calificacion_ejecutada": calificadas > 0,
        "promedio_nota": round(result.promedio_nota, 1) if result.promedio_nota else None,
        "aprobados": result.aprobados or 0,
        "total_vacantes": result.total_vacantes or 0,
        "resultados_publicados": result.publicado or False
    }
    
    return templates.TemplateResponse(
        "admin/dashboard_director.html",
        {
            "request": request,
            "usuario": usuario,
            "proceso_actual": proceso,
...
async def api_ejecutar_calificacion(
    request: Request,
    db: Session = Depends(get_db),
    usuario: dict = Depends(obtener_usuario_actual)
):
    """Ejecutar calificación de todas las hojas"""
    if usuario.get('rol') not in ['DIRECTOR', 'COORDINADOR']:
        raise HTTPException(status_code=403, detail="No tiene permisos para esta acción")
    
    try:
        data = await request.json()
        proceso = data.get('proceso', obtener_proceso_actual())
        
        # Verificar que existe gabarito
        gabarito = db.execute(text("""
            SELECT COUNT(*) as total FROM clave_respuestas WHERE proceso_admision = :proceso
        """), {"proceso": proceso}).fetchone()
        
        if gabarito.total != 100:
            return JSONResponse({"success": False, "error": "Gabarito incompleto"})
        
        # Ejecutar calificación
        #servicio = CalificacionService(db)
        #resultado = servicio.calificar_todas_las_hojas(proceso)
        resultado = calificar_hojas_pendientes(proceso, db)
        
        return JSONResponse({
            "success": True,
            "calificadas": resultado.get("calificadas", 0),
            "promedio": resultado.get("estadisticas", {}).get("nota_promedio", 0)
        })
...
async def api_publicar_resultados(
    request: Request,
    db: Session = Depends(get_db),
    usuario: dict = Depends(obtener_usuario_actual)
):
    ...
    if usuario.get('rol') != 'DIRECTOR':
        raise HTTPException(status_code=403, detail="Solo el Director puede publicar resultados")
    ...
    try:
        ...
        # Insertar registro de publicación
        db.execute(text("""
            INSERT INTO publicaciones_resultados 
            (proceso_admision, fecha_publicacion, publicado_por_id, autorizante_nombre, 
             autorizante_cargo, configuracion, estado)
            VALUES 
            (:proceso, NOW(), :usuario_id, :nombre, :cargo, :config, 'PUBLICADO')
        """), {
            "proceso": proceso,
            "usuario_id": usuario.get('id'),
            "nombre": f"{usuario.get('nombres')} {usuario.get('apellidos')}",
            "cargo": usuario.get('cargo', 'Director'),
            "config": json.dumps({
...
# ============================================================
# ============================================================================
# ENDPOINTS API - Agregar estos endpoints nuevos
# ============================================================================

@router.get("/api/stats-coordinador")
async def stats_coordinador(
    db: Session = Depends(get_db),
    usuario: dict = Depends(obtener_usuario_actual)
):
    """Estadísticas para coordinador - tiempo real"""
    proceso = obtener_proceso_actual()
    ...
    result = db.execute(text("""
        SELECT 
            ...
            (SELECT COUNT(*) FROM hojas_respuestas 
             WHERE proceso_admision = :proceso 
             AND DATE(fecha_captura) = CURRENT_DATE) as capturadas_hoy,
            (SELECT COUNT(*) FROM hojas_respuestas 
             WHERE proceso_admision = :proceso 
             AND estado IN ('PROCESADO', 'procesado')) as hojas_procesadas,
            (SELECT COUNT(DISTINCT usuario_id) 
             FROM sesiones_admin 
             WHERE expira_at > NOW()) as profesores_activos
    """), {"proceso": proceso}).fetchone()
    
    return {
        "total_postulantes": result.total_postulantes or 0,
        "capturadas_hoy": result.capturadas_hoy or 0,
        "hojas_procesadas": result.hojas_procesadas or 0,
        "profesores_activos": result.profesores_activos or 0
    }


@router.get("/api/stats-director")
async def stats_director(
    db: Session = Depends(get_db),
    usuario: dict = Depends(obtener_usuario_actual)
):
    ...
    return {
        "total_postulantes": result.total_postulantes or 0,
        "hojas_procesadas": result.hojas_procesadas or 0,
        "promedio_nota": float(result.promedio_nota) if result.promedio_nota else 0,
        "aprobados": result.aprobados or 0,
        "gabarito_registrado": (result.gabarito_count or 0) == 100,
        "calificacion_ejecutada": (result.calificadas or 0) > 0,
        "resultados_publicados": result.publicado or False
    }
